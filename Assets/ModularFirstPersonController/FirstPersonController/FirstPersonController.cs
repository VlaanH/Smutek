...
public class FirstPersonController : MonoBehaviour
{
    ...
    private Image crosshairObjectImage;
    public GameObject crosshairObjectUi;
    ...
    public KeyCode sprintKey = Settings.SelectedSettings.SprintKeyCode;
    ...
    private void Ray()
    {
        _ray = playerCamera.ScreenPointToRay(new Vector3(Screen.width / 2, Screen.height / 2));
    }

    private void DrawRay()
    {
        if (Physics.Raycast(_ray, out _hit,maxDistanceRay))
        {
            Debug.DrawRay(_ray.origin,_ray.direction * maxDistanceRay ,Color.yellow);
        }
    }

    private void Interact()
    {
        var isStopMemories = ThoughtsBox.IsStopMemories;
        if (_hit.transform != null && isStopMemories==false)
        {
            var doors = _hit.transform.GetComponent<Door>();
            var monitors = _hit.transform.GetComponent<MonitorController>();
            var movingObjects = _hit.transform.GetComponent<MovingObjects>();
            var chairs = _hit.transform.GetComponent<ChairController>();
            var lightSwitchs = _hit.transform.GetComponent<LighSwitch>();
            var microwaweDoor = _hit.transform.GetComponent<MicrowaweDoor>();
            var pc = _hit.transform.GetComponent<PcController>();
            var smartDoors = _hit.transform.GetComponent<DoorController>();
            
            var bed = _hit.transform.GetComponent<BedController>();
            
            
            var interactableObject = _hit.transform.GetComponent<InteractableObject>();
            ...
            if (doors)
            {
                ...
                {
                    Debug.DrawRay(_ray.origin, _ray.direction * maxDistanceRay, Color.green);
                    doors.Open();
                }
            ...
            if (smartDoors)
            {
                smartDoors.ChangeDoorPosition();
            }
            
            if (microwaweDoor)
            {
                microwaweDoor.ChangeMicrowaveDoor();
            }
            ...
            if (lightSwitchs)
            {
                lightSwitchs.ChangeSwitchPosition();
            }
            ...
            if (movingObjects)
            {
                movingObjects.TakeObject();
            }
        
            if (chairs)
            {
                chairs.SitOnChairForKey();
            }
        }
    ...
    public KeyCode jumpKey = Settings.SelectedSettings.JumpKeyCode;
    ...
    public void FreezingPlayer(bool isFreezing)
    {
        IsFreezing = isFreezing;
        
        playerCanMove = !isFreezing;
        isWalking = !isFreezing;
        enableJump = !isFreezing;
      
        enableCrouch = !isFreezing;
        crosshair = !isFreezing;
    ...
    private void Awake()
    {
        ...
        crosshairObjectImage = GetComponentInChildren<Image>();
    ...
    void Start()
    {
        StartCoroutine(CrosshairDelayStart());
        _protect = !crosshair;
    ...
    IEnumerator CrosshairDelayStart()
    {
    ...
    void Crosshair()
    {
        if (_protect!=crosshair)
        {
            ...
            if(crosshair)
            {
                crosshairObjectUi.gameObject.SetActive(true);
                crosshairObjectImage.sprite = crosshairImage;
                crosshairObjectImage.color = crosshairColor;
            }
            else
            {
                crosshairObjectUi.gameObject.SetActive(false);
            } 
            _protect = crosshair;
        }
...
// Custom Editor
    ...
    public override void OnInspectorGUI()
    {
        ...
        if(fpc.crosshair) 
        { 
            EditorGUILayout.PrefixLabel(new GUIContent("Crosshair Object Ui")); 
            fpc.crosshairObjectUi = (GameObject)EditorGUILayout.ObjectField(fpc.crosshairObjectUi, typeof(GameObject));
        ...
        GUILayout.Label("Ray", new GUIStyle(GUI.skin.label) { alignment = TextAnchor.MiddleLeft, fontStyle = FontStyle.Bold, fontSize = 13 }, GUILayout.ExpandWidth(true));

  
    
        fpc.maxDistanceRay = EditorGUILayout.Slider(new GUIContent("Max Distance Ray"), fpc.maxDistanceRay, .1f, 20f);
        GUI.enabled = true;
